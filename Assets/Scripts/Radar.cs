...
public class Radar : MonoBehaviour
{
    ...
    public string bombTagFilter = "Bomb";
    public float maxDist = 200;

    private void Start()
    {
        mapCenter.y = transform.parent.GetComponent<RectTransform>().rect.height - 128;
    }
    ...
    void OnGUI()
    {
        ...
        GUI.DrawTexture(new Rect(mapCenter.x - 128, mapCenter.y - 128, 256, 256), radarBG);
    ...
    private void DrawBlipsFor(string tagName)
    {
        ...
        gos = GameObject.FindGameObjectsWithTag(bombTagFilter);

        // Iterate through them
        foreach (GameObject go in gos)
        {
            drawBlip(go, bombBlip);
        }
